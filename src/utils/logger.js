...
const sensitiveFields = ['password', 'token', 'authorization', 'jwtSecret', 'sessionSecret'];
		...
		new winston.transports.Console({
			format: winston.format.combine(
				...
				winston.format.printf(info => {
					...
					return `${timestamp} [${level}]: ${message} ${Object.keys(rest).length ? JSON.stringify(rest, sensitiveDataReplacer, 2) : ''}`;
				}),
			),
		}),
